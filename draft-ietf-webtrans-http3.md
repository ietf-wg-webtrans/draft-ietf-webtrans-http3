...
endpoint can send a DRAIN_WT_SESSION (0x78ae) capsule.
...
After sending or receiving either a DRAIN_WT_SESSION capsule or a HTTP/3 GOAWAY
...
* a CLOSE_WT_SESSION capsule is either sent or received.
...
HTTP capsule {{HTTP-DATAGRAM}} of type CLOSE_WT_SESSION (0x2843). The format of
...
CLOSE_WT_SESSION has the following fields:
...
An endpoint that sends a CLOSE_WT_SESSION capsule MUST immediately send a FIN.
...
after receiving a CLOSE_WT_SESSION capsule, the stream MUST be reset with code
H3_MESSAGE_ERROR.

Cleanly terminating a CONNECT stream without a CLOSE_WT_SESSION capsule SHALL be
semantically equivalent to terminating it with a CLOSE_WT_SESSION capsule that
has an error code of 0 and an empty error string.

In some scenarios, an endpoint might want to send a CLOSE_WT_SESSION with
...
CLOSE_WT_SESSION, thus never receiving the application error data contained in
...
CLOSE_WT_SESSION properties similar to that of the QUIC CONNECTION_CLOSE
